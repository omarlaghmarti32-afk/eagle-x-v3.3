...
# Portable logging setup
LOG_DIR = os.environ.get("EAGLE_LOG_DIR", "/tmp")
os.makedirs(LOG_DIR, exist_ok=True)
LOG_FILE = os.path.join(LOG_DIR, "eagle-x.log")
        ...
        async for packet in self.monitor.start_monitoring(duration):
            ...
            if analysis.get("threat_detected"):
                ...
                if success:
                    ...
                    logger.error("Self-healing did not complete within target time.")
            else:
                logger.info("Scan normal. System Integrity: 100%. PQC active.")
...
def main():
    parser = argparse.ArgumentParser(
        description="EAGLE-X v3.3 - Quantum-Resistant Cybersecurity Titan"
    )
